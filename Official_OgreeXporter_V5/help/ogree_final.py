...
if platform == "win32":
  path_output = read_data(os.path.expanduser('~/documents') + "\\ogree_manager\\output.txt")
  path_input = read_data(os.path.expanduser('~/documents')  + "\\ogree_manager\\input.txt")
else:
  path_output = read_data(os.path.expanduser('~/Documents') + "/ogree_manager/output.txt")
  path_input = read_data(os.path.expanduser('~/Documents')  + "/ogree_manager/input.txt")
...
def import_export_json():
  ...
  while files_folder < count_files:
    ...
    try:
      ...
      if platform == "win32":
        bpy.ops.export_scene.fbx(filepath=path_output + "\\" + f_name + '.fbx')
      else:
        bpy.ops.export_scene.fbx(filepath=path_output + "/" + f_name + '.fbx')
    except:
...
def generate_asset(value, value_slot, cube_base):
  ...
  z_pos_sub.replace(",", "")
  cube.location = (float(x_pos_sub) + (x_size_sub / 2), float(y_pos_sub) + (y_size_sub / 2), float(z_pos_sub) + (z_size_sub / 2))
