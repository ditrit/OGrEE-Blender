...
def check_texture(json_component, texture_place, name_image):
    img = json_component["textures"][texture_place]
    if img.startswith('http') or img.startswith('https'):
        img_dwnd = requests.get(img).content
        file = open("resources/" +  str(name_image) + "/textures/" + texture_place + ".png", "wb")
        file.write(img_dwnd)

def create_total_mesh(name_mesh_folder, resolution, blender_path):
...
parser.add_argument('input_folder_mesh', type=str, help='Folder base for creating mesh')
parser.add_argument('-r', '--resolution', default=512, type=int, help='Texture resolution')
parser.add_argument('blender_path', default="C:\Program Files\Blender Foundation\Blender 3.1", type=str, help='Blender path')
args = parser.parse_args()
input_folder_mesh = args.input_folder_mesh
resolution = args.resolution
blender_path = args.blender_path
create_total_mesh(input_folder_mesh, resolution, blender_path)
